...
from kg_gen.models import Relation, Metadata
import os
...
def test_metadata_scenarios():
    ...
    for relation in combined_graph.relations:
        if (relation.subject == "Harry Potter" or relation.subject == "Harry") and (
            "parent" in relation.predicate.lower()
            or "son" in relation.predicate.lower()
            or "father" in relation.predicate.lower()
            or "mother" in relation.predicate.lower()
        ):
    ...
    if not merged_sources_found:
        print(
            "   ! No merged sources found (relations may not have overlapped exactly)"
        )
...
def test_direct_metadata_merging():
    ...
    print(
        f"Relations equal? {relation1 == relation2}"
    )  # Should be True (semantic equality)
    print(
        f"Relations hash equal? {hash(relation1) == hash(relation2)}"
    )  # Should be True
