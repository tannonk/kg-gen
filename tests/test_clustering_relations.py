...
def test_clustering_with_relations():
    ...
    try:
        ...
        test_relations = {
            Relation(
                subject="Josh", predicate="hasParent", object="Linda",
                metadata=Metadata(data={'source': 'family_tree.txt', 'confidence': 0.9})
            ),
            Relation(
                subject="Joshua", predicate="hasParent", object="Andrew", 
                metadata=Metadata(data={'source': 'family_records.txt', 'confidence': 0.8})
            ),
            Relation(
                subject="Ben", predicate="isBrother", object="Josh",
        ...
        print(f"   Input graph: {len(test_graph.entities)} entities, {len(test_graph.relations)} relations")
        print(f"   Sample relation metadata: {list(test_graph.relations)[0].metadata.data}")
        ...
        print(f"   Clustered graph: {len(clustered_graph.entities)} entities, {len(clustered_graph.relations)} relations")
    ...
    try:
        ...
        print(f"   Input graph: {len(old_tuple_graph.entities)} entities, {len(old_tuple_graph.relations)} relations")
        
        # Verify auto-conversion happened
        for rel in old_tuple_graph.relations:
            assert isinstance(rel, Relation), f"Expected Relation after auto-conversion, got {type(rel)}"
        ...
        print(f"   Clustered graph: {len(clustered_old_graph.entities)} entities, {len(clustered_old_graph.relations)} relations")
