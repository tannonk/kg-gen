...
def test_backward_compatibility():
    ...
    try:
        ...
        for rel in graph1.relations:
            ...
            assert isinstance(rel.metadata, Metadata), (
                f"Expected Metadata, got {type(rel.metadata)}"
            )
    ...
    try:
        ...
        for rel in graph3.relations:
            if rel.subject == "Harry Potter" and rel.predicate == "attendsSchool":
                ...
                assert (
                    "source" in rel.metadata.data and "confidence" in rel.metadata.data
                )
    ...
    try:
        ...
        graph5 = Graph(entities={"test"}, edges={"test"}, relations=invalid_relations)
