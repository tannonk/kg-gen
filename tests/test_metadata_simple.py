...
def simple_metadata_test():
    ...
    try:
        ...
        if relations_no_meta:
            ...
            print(f"   Sample relation: {sample_rel.subject} -> {sample_rel.predicate} -> {sample_rel.object}")
        ...
        rel_a = Relation(
            subject="Harry Potter", predicate="hasParent", object="James Potter",
        ...
        print(f"   Input graphs: {len(graph_a.relations)} + {len(graph_b.relations)} = {len(graph_a.relations) + len(graph_b.relations)} relations")
        print(f"   Aggregated: {len(aggregated.relations)} relations (should be 1 due to merging)")
        
        assert len(aggregated.relations) == 1, f"Expected 1 relation, got {len(aggregated.relations)}"
    ...
    try:
        ...
        test_relations = {
            Relation(subject="Harry Potter", predicate="hasParent", object="James Potter", 
                    metadata=Metadata(data={'source': 'book1.txt'})),
            Relation(subject="Harry Potter", predicate="hasParent", object="Lily Potter", 
                    metadata=Metadata(data={'source': 'book2.txt'})),
            Relation(subject="James Potter", predicate="marriedTo", object="Lily Potter",
        ...
        print(f"   Test graph: {len(test_graph.entities)} entities, {len(test_graph.relations)} relations")
