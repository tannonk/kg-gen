...
class Graph(BaseModel):
  ...
  def convert_tuple_relations(cls, v):
    ...
    for item in items:
      if isinstance(item, (tuple, list)) and len(item) == 3:
        ...
        converted.add(item)
      elif isinstance(item, dict) and 'subject' in item and 'predicate' in item and 'object' in item:
  ...
  def model_dump(self, **kwargs):
    ...
    return {
        ...
        'entity_clusters': {k: list(v) for k, v in self.entity_clusters.items()} if self.entity_clusters else None,
        'edge_clusters': {k: list(v) for k, v in self.edge_clusters.items()} if self.edge_clusters else None,
