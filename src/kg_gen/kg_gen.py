...
class KGGen:
    ...
    def generate(
        ...
        with log_step("Knowledge Graph Generation", self.logger):
            ...
            if is_conversation:
                ...
                processed_input = "\n".join(text_content)
                self.logger.info(
                    f"Processing conversation with {len(input_data)} messages"
                )
            else:
                processed_input = input_data
                self.logger.info(
                    f"Processing text input ({len(processed_input)} characters)"
                )
            ...
            if not chunk_size:
                ...
                with log_step("Text Chunking", self.logger):
                    ...
                    self.logger.info(
                        f"Split input into {len(chunks)} chunks (max size: {chunk_size})"
                    )
                ...
                with log_step("Parallel Chunk Processing", self.logger):
                    progress = ProgressTracker(
                        len(chunks), "Processing chunks", self.logger
                    )
                    ...
                    self.logger.info(
                        f"Combined results: {len(entities)} unique entities, {len(relations)} unique relations"
                    )
    ...
    def cluster(
        ...
        """
        self.logger.info("Starting graph clustering...")
        ...
        return cluster_graph(
            ...
            graph=graph,
            context=context,
            entity_cluster_context=entity_clustering_context,
    ...
    def aggregate(self, graphs: list[Graph]) -> Graph:
        ...
        with log_step("Graph Aggregation", self.logger):
            ...
            aggregated_graph = Graph(
                entities=all_entities, relations=all_relations, edges=all_edges
            )
    ...
    def get_usage_stats(self) -> Dict[str, any]:
        ...
        return {
            "models": {
                name: stats.to_dict()
                for name, stats in usage_tracker.get_all_stats().items()
            },
    ...
    def add_model_pricing(
        ...
        self.logger.info(
            f"Updated pricing for {model_name}: input=${input_price}/M, output=${output_price}/M"
        )
