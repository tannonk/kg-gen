...
class UsageTracker:
    ...
    def track_usage(self, result, model_name: Optional[str] = None, logger: Optional[logging.Logger] = None) -> None:
        ...
        """
        try:
            usage_data = result.get_lm_usage() if hasattr(result, 'get_lm_usage') else {}
            ...
            if isinstance(usage_data, dict):
                for detected_model_name, model_usage in usage_data.items():
                    ...
                    if isinstance(model_usage, dict):
                        ...
                        self._logger.warning(f"Unexpected usage data format for model {detected_model_name}: {model_usage}")
            else:
    ...
    def get_aggregate_stats(self) -> Dict[str, Any]:
        ...
        with self._stats_lock:
            ...
            # Calculate time span
            all_first_times = [stats.first_call_time for stats in self._model_stats.values() if stats.first_call_time]
            all_last_times = [stats.last_call_time for stats in self._model_stats.values() if stats.last_call_time]
            ...
            return {
                ...
                'duration_seconds': (last_call - first_call).total_seconds() if first_call and last_call else 0,
    ...
    def export_json(self, include_aggregate: bool = True) -> str:
        ...
        # Add per-model stats
        data['models'] = {name: stats.to_dict() for name, stats in self.get_all_stats().items()}
    ...
    def export_csv(self, file_path: str) -> None:
        ...
        with open(file_path, 'w', newline='') as csvfile:
            fieldnames = ['model_name', 'total_calls', 'total_prompt_tokens', 'total_completion_tokens', 
                         'total_tokens', 'total_cost', 'first_call_time', 'last_call_time', 'errors']
