...
def log_step(step_name: str, logger: Optional[logging.Logger] = None):
    ...
    try:
        ...
        duration = time.time() - start_time
        step_logger.info(
            f"Completed {step_name} in {duration:.2f}s", extra={"step": step_name}
        )
    except Exception as e:
        duration = time.time() - start_time
        step_logger.error(
...
def log_graph_stats(
    graph, operation: str = "Generated", logger: Optional[logging.Logger] = None
):
    """
    ...
    if hasattr(graph, "entity_clusters") and graph.entity_clusters:
        clustered_entities = sum(
            len(cluster) for cluster in graph.entity_clusters.values()
        )
        stats_logger.debug(
